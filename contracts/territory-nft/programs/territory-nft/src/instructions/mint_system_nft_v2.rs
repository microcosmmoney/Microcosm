...
pub struct MintSystemNftInitPda<'info> {
    #[account(mut)]
...
#[derive(Accounts)]
#[instruction(system_id: u64)]
pub struct MintSystemNftInitToken<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[derive(Accounts)]
#[instruction(system_id: u64)]
pub struct MintSystemNftFinalize<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[inline(never)]
fn create_system_metadata<'info>(
...
#[inline(never)]
fn verify_system_collection<'info>(
    token_metadata_program: AccountInfo<'info>,
    metadata: AccountInfo<'info>,
...
pub fn handler_init_pda(ctx: Context<MintSystemNftInitPda>, system_id: u64) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Step 1: System NFT PDA created: system_id={}", system_id);

    Ok(())
}

pub fn handler_init_token(ctx: Context<MintSystemNftInitToken>, system_id: u64) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Step 2: System Token created and minted: system_id={}, mint={}", system_id, nft_mint_key);

    Ok(())
}

pub fn handler_finalize(ctx: Context<MintSystemNftFinalize>, system_id: u64, uri: String) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Step 3: System NFT finalized: system_id={}, mint={}", system_id, nft_mint_key);
