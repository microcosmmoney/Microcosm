...
pub struct MintStationNftInitPda<'info> {
    #[account(mut)]
    ...
    #[account(
        ...
        seeds = [TERRITORY_NFT_SEED, &[TerritoryType::Station as u8], &station_id.to_le_bytes()],
        bump
    )]
...
#[derive(Accounts)]
#[instruction(station_id: u64)]
pub struct MintStationNftInitToken<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[derive(Accounts)]
#[instruction(station_id: u64)]
pub struct MintStationNftInit<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[derive(Accounts)]
#[instruction(station_id: u64)]
pub struct MintStationNftFinalize<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[derive(Accounts)]
#[instruction(station_id: u64)]
pub struct MintStationNft<'info> {
    #[account(mut)]
    pub authority: Signer<'info>,
...
#[inline(never)]
fn create_station_metadata<'info>(
    ...
    let name = format!("{}{}", STATION_NAME_PREFIX, station_id);
    let data = DataV2 {
        name,
...
#[inline(never)]
fn create_station_edition<'info>(
    token_metadata_program: AccountInfo<'info>,
...
#[inline(never)]
fn verify_station_collection<'info>(
    token_metadata_program: AccountInfo<'info>,
    metadata: AccountInfo<'info>,
...
pub fn handler_init_pda(ctx: Context<MintStationNftInitPda>, station_id: u64) -> Result<()> {
    let clock = Clock::get()?;

    require!(station_id > 0, TerritoryError::InvalidTerritoryId);

    let recipient_key = ctx.accounts.recipient.key();

    let territory_nft = &mut ctx.accounts.territory_nft;
    territory_nft.territory_type = TerritoryType::Station;
    territory_nft.territory_id = station_id;
    ...
    msg!("Step 1: Territory NFT PDA created: station_id={}", station_id);

    Ok(())
}

pub fn handler_init_token(ctx: Context<MintStationNftInitToken>, station_id: u64) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Step 2: Token created and minted: station_id={}, mint={}", station_id, nft_mint_key);

    Ok(())
}

#[allow(dead_code)]
pub fn handler_init(ctx: Context<MintStationNftInit>, station_id: u64) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Step 1: Station NFT initialized: station_id={}, mint={}", station_id, nft_mint_key);

    Ok(())
}

pub fn handler_finalize(ctx: Context<MintStationNftFinalize>, station_id: u64, uri: String) -> Result<()> {
    let clock = Clock::get()?;
    ...
    collection_config.total_stations = collection_config.total_stations.saturating_add(1);
    collection_config.updated_at = clock.unix_timestamp;

    msg!("Step 2: Station NFT finalized: station_id={}, mint={}", station_id, nft_mint_key);

    Ok(())
}

#[allow(dead_code)]
pub fn handler(ctx: Context<MintStationNft>, station_id: u64, uri: String) -> Result<()> {
    let clock = Clock::get()?;
    ...
    msg!("Station NFT minted: station_id={}, mint={}", station_id, nft_mint_key);
